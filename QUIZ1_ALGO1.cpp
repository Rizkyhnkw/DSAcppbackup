...
int main() {
    ...
    // Menghitung jumlah pulsa per 15 detik
    int p=d/ 15;

    // Menghitung tarif sesuai kode
    if (kode_area =="021") {
        t= 250;
    ...
    // Menghitung total tarif
     (d>3600)? total_t=(t*p)-(0.5*t*(d-3600) / 15) :total_t= t*p;

    // Output hasil
    cout << "Durasi pemakaian     : " << d / 60 << " menit " << d % 60 << " detik"<< endl;
    cout << "jumlah pulsa terpakai: " << p << endl;
    cout << "Tarif per pulsa      : Rp " << t << endl;
    cout << "Total tarif          : Rp " << total_t << endl;

    return 0;
//tugas quiz1 (nama:rizky ramadhan)>>S1 ilkom
}
